...
var displayMap = function displayMap(locations, token, styles) {
...
var stripe = Stripe('pk_test_w8IMpzMoOzcMjdnjfWobiBWc00hgcSOydj');
...
if (mapBox && mapStuff) {
  ...
  var stuff = mapStuff.dataset.stuff.split(' ');
  var token = stuff[0];
  var styles = stuff[1];
  (0, _mapbox.displayMap)(locations, token, styles);
}
...
if (bookBtn) bookBtn.addEventListener('click', function (e) {
  e.target.textContent = 'Processing...'; // button.btn.btn--green.span-all-rows#book-tour(data-tour-id=`${tour.id}`) Book tour now!
